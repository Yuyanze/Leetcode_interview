...
def DenoisebyDBSCAN(CenterData,CenterX, CenterY, EPS, MIN_SAMPLES):
    ...
    if max(label_pred) == -1:  # 如果只有异常点，不进行处理
        ...
        minIndexy = list(map(denoisey.index, heapq.nsmallest(5, denoisey)))  # 提取Y值最小的10个点
...
def EstimateBladeTipClearance(ImagePath):
    ...
    for ImageName in ImageNames[2:]:
        ...
        morphologyone  = sm.binary_dilation(binaryone,selem=sm.square(6)).astype(np.uint8)

        # plt.figure('mor',figsize=(8,8))
        # plt.subplot(121)
        # plt.title("original image")
        # plt.imshow(binaryone,plt.cm.gray)
        # #
        # plt.subplot(122)
        # plt.title("morphology image")
        # plt.imshow(morphologyone,plt.cm.gray)

        contours, hierarchy = cv2.findContours(morphologyone, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        ...
        # TipX, TipY, CenterData = DenoisebyDBSCAN(CenterData, CenterX, CenterY,15,10)#  Second denoise
        #Endtimer = time()
        ...
        cv.waitKey(10)
        currentframegry, nextframegry = nextframegry, frameTempbuffergry
        currentframe, nextframe = nextframe, frameTempbuffer
    return TipX, TipY
