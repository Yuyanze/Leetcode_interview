...
def DenoisebyDBSCAN(CenterData, EPS, MIN_SAMPLES):
    ...
    if max(label_pred)== -1:  # 如果只有异常点，不进行处理
        ...
        minIndexy = list(map(denoisey.index, heapq.nsmallest(10, denoisey)))  # 提取Y值最小的10个点
...
def EstimateBladeTipClearance(firstImage, secondImage, ThirdImage):
    #
    currentframe = firstImage.copy()
    nextframe    = secondImage.copy()
    ...
    #for ImageName in ImageNames[2:]:
        ...
        #print((count, ImageName))
    # 下一帧
    afternextframe = ThirdImage.copy()
...
if __name__ == '__main__':
    ...
    # Step 2：利用帧差法求风机轮廓区域
    # 第一帧
    firstImage = cv.imread(ImageNames[7])
    # 第二帧
    secondImage = cv.imread(ImageNames[8])

    ThirdImage  = cv.imread(ImageNames[9])
